...
node
{
   ...
   stage('SCM checkout') {
        checkout([$class: 'GitSCM', branches: [[name: 'Test/test']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'Git_hub_id', url: 'https://github.com/abes-esr/abes-hello-front.git']]])
   }
    ...
    stage ('deploy to raiponce'){
       echo 'deployment test server started'
        ...
        sshagent(credentials: ['raiponce1-test-ssh-key']) { //one key per tomcat
            ...
            sh 'ssh -tt devel@raiponce1-test.v3.abes.fr  "cd /var/www/html/hello/ && rm -rf -d js && rm -rf -d css"'
            sh 'scp -r dist/* devel@raiponce1-test.v3.abes.fr:/var/www/html/hello/'
